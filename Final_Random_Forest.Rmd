...
use_ranger(muhat~ X1 +X2, data = forest_train)
...
ranger_recipe <- 
  recipe(formula = muhat ~ X1 + X2, data = forest_train) 
...
set.seed(673)
doParallel::registerDoParallel(cl)# set multithreading :O
ranger_tune <-
...
collect_predictions(muhat_rf) %>%
  ...
  geom_abline(lty= 5, color = 'red', size = .75) +
  geom_point(alpha = .05, color = 'midnightblue') +
  coord_fixed()
...
use_ranger(Vhat~X1 + X2, data = forest_train)

ranger_recipe <- 
  recipe(formula = Vhat ~ X1 + X2, data = forest_train) 
...
Vhat_rf = last_fit(final_Vhat_rf, forest_split)
Vhat_rf
```


```{r}
collect_metrics(Vhat_rf)
```


```{r}
collect_predictions(Vhat_rf) %>%
  ggplot(aes(Vhat, .pred)) + 
  geom_abline(lty= 2, color = 'red', size = .75) +
  geom_point(alpha = .09, color = 'midnightblue') +
  coord_fixed()
